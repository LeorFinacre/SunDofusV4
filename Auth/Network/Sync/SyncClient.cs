...
namespace SunDofus.Auth.Network.Sync
{
    class SyncClient : Master.TCPClient
    {
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Send to Sync [{0}] : {1}", IP, message);
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Received from sync [{0}] : {1}", IP, datas);
        ...
        private void Disconnected()
        {
            ...
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "New closed sync connection <{0}> !", this.IP);
        ...
        private void Authentication(string[] datas)
        {
            ...
            if (Entities.Requests.ServersRequests.Cache.Any(x => x.ID == serverId && x.IP == serverIp && x.Port == serverPort && x.State == 0))
            {
                ...
                Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Sync <{0}> authentified !", this.IP);
            }
            else
                Disconnect();
        }
