...
namespace SunDofus.Auth.Network.Auth
{
    class AuthClient : Master.TCPClient
    {
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Send to [{0}] : {1}", this.IP, message);
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Receive from client [{0}] : {1}", this.IP, datas);
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "New closed client connection <{0}> !", this.IP);
        ...
        private void CheckAccount(string datas)
        {
            ...
            if (requestedAccount != null && Utilities.Basic.Encrypt(requestedAccount.Password, key) == password)
            {
                ...
                Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Client <{0}> authentified !", Account.Pseudo);
        ...
        {
            if (datas.Contains(Utilities.Config.GetString("LOGIN_VERSION")))
                State = AccountState.OnCheckingAccount;
            else
            {
                Send(string.Concat("AlEv", Utilities.Config.GetString("LOGIN_VERSION")));
                this.Disconnect();
            }
        }
