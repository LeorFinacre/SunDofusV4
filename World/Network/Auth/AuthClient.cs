...
namespace SunDofus.World.Network.Auth
{
    class AuthClient : Master.TCPClient
    {
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Errors, "Cannot connect to AuthServer because ", exception.ToString());
        }
        ...
        {
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Connection with the AuthServer closed !");
            timer.Start();
        }
        ...
        private void ParsePacket(string datas)
        {
            ...
            {
                Utilities.Logger.Write(Utilities.Logger.LoggerType.Errors, "Cannot parse AuthServer's packet ({0}) because : {1}", datas, e.ToString());
            }
        }

        private void ReceiveHelloConnect()
        {
            var objects = new object[] { Utilities.Config.GetInt32("SERVERID"), Utilities.Config.GetString("SERVERIP"),
                            Utilities.Config.GetInt32("SERVERPORT"), this.Model.PassKey };
        ...
        {
            isLogged = true;
            Utilities.Logger.Write(Utilities.Logger.LoggerType.Debug, "Connected with the AuthenticationServer !");
